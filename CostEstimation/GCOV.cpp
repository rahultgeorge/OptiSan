...
uint64_t GCOVFile::getCount(Instruction *Inst) const
{
    ...
    if (!F)
    {
        ...
        errs() << "Warning: could not find function:" << ParentF->getName() << " in GCOV data\n";
        return 0;
    }
    //   errs()<<"Found function:"<<F->name<<"\n";
    ...
    if (ParentF->size() + 1 == F->getNumBlocks())
    {
        ...
        {
            if (!(((isa<ReturnInst>(ParentBI->getTerminator()) &&
                    BI->getNumDstEdges() == 1) ||
        ...
        // PDQ : This should only be checked in case it is not the last block (non GCOV). As the last bb in the function will have no edges but the the same bb in gcov will have one edge as there is an extra return bb added by a compilation flag
        if (ParentBI->getTerminator()->getNumSuccessors() != 0)
            assert(ParentBI->getTerminator()->getNumSuccessors() ==
                       BI->getNumDstEdges() &&
                   "CFG mismatch: dst edges(outside loop)");
        return BI->getCount();
    }
    errs() << *Inst << "|" << (Inst->getFunction()->getName()) << "\n";
    errs() << "\t # BBs in program:" << ParentF->size() << "\n";
    errs() << "\t # BBs in gcov function:" << F->getNumBlocks() << "\n";
    errs() << "\t BB mismatch:" << F->name << ":" << ParentF->getName() << "\n";
    return 0;
}
...
const GCOVFunction *GCOVFile::getFunction(const Function *F) const
{
    ...
    bool searchForMain = false;
    bool hasDebugInfo = false;
    std::string gcovDBGID;
    std::string functionDBGID;
    //    if(F->getName().equals("main_original"))
    //        searchForMain=true;
    DISubprogram *functionDISubprogramInfo = F->getSubprogram();
    if (functionDISubprogramInfo)
        hasDebugInfo = true;
    ...
    for (const auto &Fptr : Functions)
    {
        ...
        // errs()<< "File name:"<<Fptr->getFilename().str()<<F->getSubprogram()<<"\n";
        //  errs()<<"Function in function small vector:"<< Fptr->getName()<<","<<
        //  F->getName() <<"\n";
        if (hasDebugInfo)
        {
            gcovDBGID = Fptr->fileName + ":" +
                        std::to_string(Fptr->getLineNumber());

            functionDBGID = functionDISubprogramInfo->getFilename().str() + ":" +

                            std::to_string(functionDISubprogramInfo->getLine());

            if (gcovDBGID.compare(functionDBGID) == 0)
            {
                // errs() << "GCOV dbg id:" << gcovDBGID << "\n";
                // errs() << "\t" << functionDBGID << "\n";
                return Fptr.get();
            }
        }
        else
        {
            ...
            if (Fptr->name == F->getName())
                return Fptr.get();
            //            //Baggy hack to deal with main and main_original
            //            if (searchForMain && Fptr->name == "main")
            //                return Fptr.get();
            //            else if (Fptr->name == F->getName())
            //                return Fptr.get();
        }
    }
    ...
    while (Buff.readEdgeTag())
    {
        ...
        for (uint32_t i = 0, e = EdgeCount; i != e; ++i)
        {
            ...
            if (!Buff.readInt(Dst))
                return false;
            Edges.push_back(
                make_unique<GCOVEdge>(*Blocks[BlockNo], *Blocks[Dst]));
...
FileInfo::openCoveragePath(StringRef CoveragePath)
{
    ...
    std::error_code EC;
    auto OS =
        make_unique<raw_fd_ostream>(CoveragePath, EC, sys::fs::OF_Text);
    if (EC)
    {
        errs() << EC.message() << "\n";
        return make_unique<raw_null_ostream>();
