...
void pdg::GenericGraph::dumpGraph()
{
  ...
  for (auto node_iter = begin(); node_iter != end(); ++node_iter)
  {
    ...
    {
      errs() << "edge: " << out_edge << " / "
             << "src[" << out_edge->getSrcNode() << "] / "
             << " dst[" << out_edge->getDstNode() << "]"
...
/**
 * OptiSan- Stack data indirect uses through aliases
 */
bool pdg::ProgramGraph::isIndirectUse(pdg::Node &src, pdg::Node &dst)
{

  return canReach(src, dst);
}
