...
void process_mem_usage()
{
  ...
  // Print memory usage
  errs() << "Memory usage: " << usage.ru_maxrss << " KB\n";
}
...
bool pdg::ProgramDependencyGraph::runOnModule(Module &M)
{
  ...
  if (!_PDG->getAddressTakenStackObjects().empty())
  {
    errs() << "# Address taken stack variables:" << _PDG->getAddressTakenStackObjects().size() << "\n";
...
void pdg::ProgramDependencyGraph::connectGlobalVarWithUses()
{
  ...
  // OptiSan : Want to capture RAW Dependencies across global uses (overapproximating)
  for (auto &global_var : _module->getGlobalList())
  {
    ...
    for (auto user : global_var.users())
    {
      ...
      if (user_node == nullptr)
        continue;
      if (StoreInst *userInst = dyn_cast<StoreInst>(user))
      {
        // Overapproximating - connecting it to all reads (or other uses of the same global variable)
        if (userInst->getPointerOperand() != &global_var)
          continue;
        for (auto otherUser : global_var.users())
        {
          Node *other_user_node = _PDG->getNode(*otherUser);
          if (other_user_node == nullptr)
            continue;

          if (user != otherUser)
          {
            if (Instruction *useInst = dyn_cast<Instruction>(otherUser))
            {
              user_node->addNeighbor(*other_user_node, EdgeType::GLOBAL_DEP);
              FunctionWrapper *func_w = getFuncWrapper(*useInst->getFunction());
              for (auto retIns : func_w->getReturnInsts())
              {
                Node *retNode = _PDG->getNode(*retIns);
                if (!retNode)
                  continue;
                // OptiSan - (Hacky) way to enable easy intra procedural computation of usable targets
                other_user_node->addNeighbor(*retNode, EdgeType::GLOBAL_DEP);
              }
            }
          }
        }
...
void pdg::ProgramDependencyGraph::connectCallerAndCallee(CallWrapper &cw, FunctionWrapper &fw)
{
  ...
  {
    errs() << "Failed to add call edge because no  node:" << call_site_node << "->" << func_entry_node << "\n";
    return;
  }
  call_site_node->addNeighbor(*func_entry_node, EdgeType::CONTROLDEP_CALLINV);

  // Step 2: connect both control/data return edges of callee to the call site
...
void pdg::ProgramDependencyGraph::connectCallerAndCalleeTrees(CallWrapper &cw, FunctionWrapper &fw)
{
  // step 1: connect actual in -> formal in, formal out -> actual out
  auto actual_arg_list = cw.getArgList();
  auto formal_arg_list = fw.getArgList();
  assert(actual_arg_list.size() == formal_arg_list.size() &&
         "cannot connect tree edges due to inequal arg num! (connectCallerandCallee)");
  ...
  // step2: connect return value actual in -> formal in, formal out -> actual out
...
void pdg::ProgramDependencyGraph::connectInterprocDependencies(Function &F)
{
  ...
  bool isActualTreeEmpty = false;
  bool shouldConstructParameterTrees = !_PDG->getAddressTakenStackObjects().empty();
  for (auto call_inst : call_insts)
  {
    if (_PDG->hasCallWrapper(*call_inst))
    {
      ...
      /*OptiSan: In case di information is missing and therefore there is no tree for an argument we should still connect
       * the caller and the callee via data and control edges. We need to therefore connect them before we return
       */

      if (shouldConstructParameterTrees)
      {
        isActualTreeEmpty = false;
      ...
      if (call_w->getCalledFunc() != nullptr)
      {
        ...
        if (!isActualTreeEmpty && shouldConstructParameterTrees)
          connectCallerAndCalleeTrees(*call_w, *called_func_w);
      }
      else
      {
        // indirect call
