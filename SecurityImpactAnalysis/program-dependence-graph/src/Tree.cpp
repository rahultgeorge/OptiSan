...
void pdg::TreeNode::computeDerivedAddrVarsFromParent()
{
  ...
  if (grand_parent_node != nullptr && _parent_node->getDIType() && grand_parent_node->getDIType() && dbgutils::isStructType(*_parent_node->getDIType()) && dbgutils::isStructPointerType(*grand_parent_node->getDIType()))
  {
  ...
  if (_parent_node->getDIType() && dbgutils::isStructType(*_parent_node->getDIType()))
    is_struct_field = true;

  for (auto base_node_addr_var : base_node_addr_vars)
  {
    if (!base_node_addr_var)
      continue;
