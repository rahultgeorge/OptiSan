...
private:
    ...
    // These are unsafe pointers for which we don't have points to info
    std::set<llvm::Value *> mayPointToStackUnsafePtr;
...
public:
    ...
    void addUnsafePointerArithmetic(llvm::Instruction *instruction);

    void addMayPointToStackPtr(llvm::Value *ptr);

    void clearUnsafeMemoryAccessesAndMappings()
    {
        unsafeStackMemoryAccesses.clear();
        unsafeMemoryAccessToMemoryObject.clear();
        unsafeMemoryAccessToUnsafePointerArithmetic.clear();
    }

    void clearMayPointToStackPtrs()
    {
        mayPointToStackUnsafePtr.clear();
    }

    std::set<llvm::Instruction *> getUnsafeMemoryAcceses() { return unsafeStackMemoryAccesses; };

    std::set<llvm::Value *> getMayPointToStackUnsafePtrs() { return mayPointToStackUnsafePtr; };

    std::set<llvm::Instruction *> getUnsafePointerArithmeticInstructions() { return unsafePointerArithemticInstructions; };

    std::set<llvm::Instruction *> getUnsafeObjectsForAccess(llvm::Instruction *memoryAccess);

    std::set<llvm::Instruction *> getUnsafeObjectsForPointerArithmetic(llvm::Instruction *memoryAccess);
