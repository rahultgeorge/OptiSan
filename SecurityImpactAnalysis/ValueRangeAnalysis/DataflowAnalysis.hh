  ...
  public:
    ...
    computeForwardDataflow()
    {
      while (!contextWork.empty())
      {
        ...
        // llvm::errs() << " Computing forward data flow for: " << function->getName() << "\n";
    ...
    computeForwardDataflow(llvm::Function &f, const Context &context)
    {
      ...
      // llvm::errs() << "First compute the initial outgoing state of all instructions:" << f.getName() << "\n";

      FunctionResults results = allResults.FindAndConstruct(context).second.FindAndConstruct(&f).second;
      ...
      // llvm::errs() << "Second Add all blocks to the worklist in topological order for efficiency \n";
