...
namespace
{
	...
	private:
		...
		// CHANGE-1 Maps instruction addresses to the original instruction strings (prechange) - SVF
				...
				if (II->getType()->isPointerTy())
				{
					...
					if (varinfo != NULL)
					{
																 ...
																 (isa<Instruction>(varinfo->size) && ((Instruction *)varinfo->size)->getParent() != II->getParent())))
						{
							...
							if (varinfo->explicitSizeVariable->getType()->isPointerTy())
							{
		...
		bool runOnModule(Module &M) override
		{
			...
			errs() << "\n\n#############\n MODULE: " << M.getName() << '\n';
			// CHANGE 2
			/* To deal with the modified IR we record the original instruction for all the IR SSA pointer variables
			 * Macros are inconvenient otherwise DBG ID would be sufficient (also with all this instrumentation it could get corrupt)
			 * */
			...
			/* 	for (auto i = M.begin(), e = M.end(); i != e; ++i)
				{
					...
					{
						tempInst = dyn_cast<GetElementPtrInst>(&*it);
						if (tempInst && (!(tempInst->hasAllZeroIndices())))
						{
						...
						// 	rso.flush();
						// 	rso << *allocaInst;
						// 	instructionAddressesToStringsMap[allocaInst] = instructionString;
						// }
					}
				}
			*/
			errs() << " PTA construction\n";
		...
		void identifyDifferentKindsOfUnsafePointers()
		{
				 ...
				 it != TheState.Variables.end(); it++)
			{
					...
					it->second.classification == NesCheck::VariableStates::Seq)
				{
					...
					if (instruction)
					{
						...
						// CHANGE 3- Based on points to check count the unsafe ptrs
						if (isUnsafeStackPointer)
						{
							unsafeStackPointerCount =
								unsafeStackPointerCount + isUnsafeStackPointer;
							...
							{
								stackSeqPointerCount =
									stackSeqPointerCount + isUnsafeStackPointer;
								stackSeqPointerSet.insert(unsafePointer);
							}
							if (it->second.classification == NesCheck::VariableStates::Dyn)
							{
								stackDynPointerCount =
									stackDynPointerCount + isUnsafeStackPointer;
			...
			// CHANGE 4 - Pass the entire unsafe pointer set, along with IR map and points to info
			valueRangeAnalysis(CurrentModule, unsafeStackPointerSet, instructionAddressesToStringsMap, ptaw);
		}
	};
}
